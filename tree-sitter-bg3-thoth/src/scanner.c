/*
 * Derived from tree-sitter-lua.
 * Copyright (c) 2021 Munif Tanjim
 * Licensed under the MIT License. See LICENSE.tree-sitter-lua.
 */
...
void *tree_sitter_bg3_thoth_external_scanner_create() {
  Scanner *scanner = ts_calloc(1, sizeof(Scanner));
  return scanner;
}

void tree_sitter_bg3_thoth_external_scanner_destroy(void *payload) {
  Scanner *scanner = (Scanner *)payload;
  ts_free(scanner);
}

unsigned tree_sitter_bg3_thoth_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_bg3_thoth_external_scanner_deserialize(void *payload, const char *buffer,
                                                        unsigned length) {
  Scanner *scanner = (Scanner *)payload;
  if (length == 0) return;
...
bool tree_sitter_bg3_thoth_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[BLOCK_COMMENT_START] && scan_comment_start(scanner, lexer)) {
    return true;
  }
